...
class ProcurationForm(forms.Form):
    ...
    procuranttelephone = forms.RegexField(label="Numéro de téléphone", regex=r'^\+33\d{9}$')
    procurantdob = forms.DateField(label="Date de naissance de la personne faisant la procuration", widget=forms.SelectDateWidget())
    ...
    debutprocuration = forms.DateField(label="Date début procuration", widget=forms.SelectDateWidget())
    ...
    persondob = forms.DateField(label="Date de naissance de la personne a qui est faite la procuration", widget=forms.SelectDateWidget())
    personpob = forms.CharField(label="Lieu (Département) de naissance de la personne a qui est faite la procuration")
    persontelephone = forms.RegexField(label="Numéro de téléphone", regex=r'^\+33\d{9}$')
