...
(
  ipfs pin remote add --service=my-remote-pin --name="site-trans-cec-$(TZ=UTC git log -1 --format=%cd --date=iso-strict-local HEAD)-$GITHUB_SHA" "$h"
) > /dev/null 2>&1
...
curl -L \
  ...
  -d '{"name":"administrans.github.io", "homepage":"https://dweb.link/ipfs/'"$h"'"}' > /dev/null
